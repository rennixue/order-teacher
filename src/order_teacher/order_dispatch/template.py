...
class Template(metaclass=ABCMeta):
    ...
    def render(self, vals: Mapping[str, Any] | None = None, **kwargs: Any) -> str:
        if vals is None:
            vals = kwargs
        # NOTE Incorrect if template inheritance is involved.
...
class FsTemplateManager(TemplateManager):
    ...
    def _load_paths(cls, path: str | os.PathLike[str]) -> dict[str, Path]:
        ...
        for fpath in fpaths:
            ...
            name_to_path[name.replace(os.sep, "/")] = fpath
        return name_to_path
...
class JinjaTemplateManager(FsTemplateManager):
    ...
    def _load_template(self, name: str) -> JinjaTemplate:
        ...
        load_name = str(path.relative_to(self._root)).replace(os.sep, "/")
        # NOTE Incorrect if template inheritance is involved.
        file_content = path.read_text(encoding="utf-8").rstrip()
        parsed_content = self._env.parse(file_content, load_name, path.name)
        varnames = jinja2.meta.find_undeclared_variables(parsed_content)
        # varnames: set[str] = set()
        return JinjaTemplate(self._env.get_template(load_name), varnames)

    def load_all_templates(self) -> dict[str, JinjaTemplate]:
        d: dict[str, JinjaTemplate] = {}
        for name in self._name_to_path.keys():
            d[name] = self._load_template(name)
        return d
